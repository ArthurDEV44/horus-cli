...
/**
 * User-level settings stored in ~/.horus/user-settings.json
 * These are global settings that apply across all projects
 */
export interface UserSettings {
  apiKey?: string; // Horus API key
...
/**
 * Project-level settings stored in .horus/settings.json
...
export class SettingsManager {
  ...
  private constructor() {
    // User settings path: ~/.horus/user-settings.json
    ...
    // Project settings path: .horus/settings.json (in current working directory)
  ...
  /**
   * Load user settings from ~/.horus/user-settings.json
   */
  ...
  /**
   * Save user settings to ~/.horus/user-settings.json
   */
  ...
  /**
   * Load project settings from .horus/settings.json
   */
  ...
  /**
   * Save project settings to .horus/settings.json
   */
  ...
  public getCurrentModel(): string {
    ...
    return DEFAULT_PROJECT_SETTINGS.model || "devstral:24b";
  }
  ...
  public getApiKey(): string | undefined {
    ...
    const envApiKey = process.env.HORUS_API_KEY;
    if (envApiKey) {
      return envApiKey;
    }
  ...
  public getBaseURL(): string {
    // First check environment variable
    const envBaseURL = process.env.HORUS_BASE_URL;
    if (envBaseURL) {
      return envBaseURL;
    }
